...
new_data = dataset.drop('user_id', axis=1)
...
user_lookup = tf.keras.layers.StringLookup(
    vocabulary=list(new_data['product_id'].unique()),  # List of unique user_ids
    mask_token=None,
    num_oov_indices=0,
    output_mode='int',  # Output integers
    name='product_lookup'
...
new_data['encoded_product_id'] = user_lookup(new_data['product_id'])

new_data = new_data.drop('product_id', axis=1)
new_data.info()
...
recommendations_user, recommendations_top_rated = recommend_products(encoded_user_id_example, dataset_example)

print(f"\nRekomendasi untuk {(recommendations_user).to_string(index=False)} ! :" +"\n"+ f"\n{recommendations_top_rated}")
